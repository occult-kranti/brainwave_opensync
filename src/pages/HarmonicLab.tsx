...
function SignalPlot({ recipe }: { recipe: HarmonicRecipe }) {
  ...
  return <div className="harmonic-signal">
    ...
    <p className="harmonic-meta">Base chord components, before envelopes and output gain. Coincident partials are drawn separately, not summed. A progression transposes this pattern. These are generated components, not a microphone or EEG reading.</p>
  </div>;
}
...
export default function HarmonicLab() {
  ...
  return <div className="harmonic-page">
    <header className="harmonic-heading"><div><div className="harmonic-eyebrow"><Music2 size={16} /> COMPOSE / LISTEN / COMPARE</div><h1>Harmonic Lab</h1><p>Find your tuning. Shape the overtones. Hear the difference.</p></div><div className="flex items-center gap-2"><GradeBadge grade="A" citation={{ verdict: 'A for frequency arithmetic and audio synthesis only.', summary: 'A tuning choice is not evidence of a physiological effect.', source: HARMONIC_SOURCES[0].url }} /><InfoPopover featureId="harmonic-composer" /></div></header>
    ...
    <div className="harmonic-workspace">
      ...
      <section className="harmonic-panel harmonic-preview" aria-labelledby="listen-heading"><h2 id="listen-heading">02 <span>Hear what you build</span></h2><SignalPlot recipe={recipe} />
        ...
        {blocked && <p className="harmonic-callout" role="status">{governor.infantMode ? 'Harmonic Lab is unavailable in infant mode. The Safety page controls this setting.' : panicked ? 'Panic is active. Use the global resume control before playing.' : running ? 'Stop the Studio session before previewing here.' : 'The Studio is muted. Unmute before playing a preview.'}</p>}
    ...
    <footer className="harmonic-panel harmonic-sources"><h2>What “right frequency” means here</h2><p>A frequency is right for a chosen tuning when it matches that tuning’s ratios. Musical preference and claimed changes in consciousness are different questions. This lab calculates and plays sound; it does not measure consciousness.</p><div>{HARMONIC_SOURCES.map((s) => <p key={s.url}><a href={s.url} target="_blank" rel="noreferrer">{s.label}</a><span className="harmonic-meta">{s.scope}</span></p>)}</div><Link to="/sonic-lab">More generators in Sonic Lab</Link><Link to="/sample-lab">Analyze an exported recording</Link></footer>
  </div>;
}
